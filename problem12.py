...
# ...
# {'vegetable oil', 'vegetable stock', 'barley malt', 'tofu', 'fresh basil',
...
def clean_ingredients(dish_name, dish_ingredients):
    x = (dish_name, set(dish_ingredients))
    return x

def check_drinks(drink_name, drink_ingredients):
    if len(ALCOHOLS & set(drink_ingredients)) == 0:
        x = f"{drink_name} Mocktail"
        return x
    x = f"{drink_name} Cocktail"      
    return x
...
def categorize_dish(dish_name, dish_ingredients):
    ...
    if dish_ingredients_set.issubset(VEGAN):
        ...
        x = f"{dish_name}: OMNIVORE"
    return x
    
def tag_special_ingredients(dish):
    dish_set = set(dish[1])
    x = dish_set & SPECIAL_INGREDIENTS 
    y = (dish[0],x)
    return y
...
def separate_appetizers(dishes, appetizers):
    ...
    dish_names_with_appetizers_removed = dishes_set - appetizers_set
    return dish_names_with_appetizers_removed


def singleton_ingredients(dishes, intersection):
    x = compile_ingredients(dishes)
    y = x - intersection
    return y

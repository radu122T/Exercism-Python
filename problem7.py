...
# In Black Joe, speed is important. Elyse thinks she can "shortcut" the traditional calculation to get an average-like number by taking the average of the first and last card values in the hand - or by using the "middle" card value (the median) of the hand. She is hoping one of these calculations might come close to the average, but be quicker to calculate during game play. She'd like you to create a function to test her theory out.

# Implement a function approx_average_is_average(<hand>) that returns a Boolean. The function should return True if one (or both) of Elyse's approximations yields the same value as calculating the "full" average of a hand. The function should return False if neither approximation formula yields the same value as the "full" average. For the sake of a simple median, we are going to assume the hand always has an odd number of card values.
